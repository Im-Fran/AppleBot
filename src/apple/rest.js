...
const request_changelog = async (audience, model, documentationId, id) => {
    ...
    console.log(`CHANGELOG: https://gdmf.apple.com/v2/assets POST: ${JSON.stringify(postData)}`)
    ...
    if(!json.Assets[0]) return 'No changelog found.'
    ...
    return sanitized.length > 4000 ? sanitized.substring(0, 4000) + '...\n\n*[Release notes have been truncated]*' : sanitized;
};
...
const request_update = async (audience, build, model, productType, productVersion, isBeta, id) => {
    ...
    const postData = {
        ...
        ClientVersion: 2,
        AssetType: id === 'macos' ? 'com.apple.MobileAsset.MacSoftwareUpdate' : "com.apple.MobileAsset.SoftwareUpdate",
        BuildVersion: config.devices.ios.build,
        HWModelStr: config.devices.ios.model,
        ProductType: config.devices.ios.prodtype,
        ProductVersion: config.devices.ios.version,
    };
    console.log(`UPDATE: https://gdmf.apple.com/v2/assets POST: ${JSON.stringify(postData)}`)
    const agent = new https.Agent({
        rejectUnauthorized: false,
    })
    let res = await axios.post('https://gdmf.apple.com/v2/assets', postData, { httpsAgent: agent }).catch(error => {
        console.log(`Failed to fetch ${id} Update (91): ${error}`)
    })
    ...
    if(json.Assets[0]){
        ...
        return {
            ...
            os_postdate: json.Assets[0].PostingDate,
            os_download: json.Assets[0].__BaseURL + json.Assets[0].__RelativePath
        };
    } else {
        console.log('No update asset found (121)')
        return null
    }
}
